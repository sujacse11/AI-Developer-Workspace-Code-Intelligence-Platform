...
import { Code2, LayoutDashboard, ShieldAlert, LogOut, Sparkles, FolderCode, User as UserIcon, ShieldCheck } from 'lucide-react';
          ...
          {user ? (
            <div className="flex items-center space-x-3 pl-3 border-l border-slate-800">
              <button
                ...
                className="flex items-center space-x-2 p-1.5 rounded-xl hover:bg-slate-800/80 transition text-left group border border-transparent hover:border-slate-700"
              >
                <div className="w-7 h-7 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-xs font-bold text-white shadow group-hover:scale-105 transition-transform">
                  {user.username[0].toUpperCase()}
                </div>
                <div className="hidden sm:flex flex-col">
                  <span className="text-xs font-semibold text-slate-200 group-hover:text-cyan-400 transition leading-tight flex items-center space-x-1">
                    <span>{user.username}</span>
                    {user.kyc_verified && <ShieldCheck className="w-3 h-3 text-cyan-400 inline" />}
                  </span>
